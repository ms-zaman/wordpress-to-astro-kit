...
|                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Here**       | The Astro app and its seams · the content contracts · the twenty gates in [scripts/](scripts/), each with its own suite · `kit:init` · the playbook, checklist and governance templates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Not yet**    | Nothing in the procedure is unsupported. `pnpm content:transform` now turns a capture of WordPress's own shapes — posts, pages, categories, tags, authors — into the content tree. What it deliberately leaves to you is a CUSTOM type or taxonomy, because a profile is a decision about what should become a public URL, and a schema you have extended is one only you can map. [scripts/content-transform/](scripts/content-transform/README.md) states what it refuses and why.                                                                                                                                                                                                  |
...
| **Principle**  | **Identity is not a slug, and downstream integrity cannot compensate for upstream entity loss.** Four defects in this kit were one shape: a valid entity disappeared or overwrote another before any gate could see it — twice at an Astro loader deriving identity from content, once at an unowned directory, once at two files producing one identity while every gate agreed with itself. Uniqueness is now checked at the earliest boundary that can see both sides: the filesystem for content, the resolver for routes, the build audit for outputs. [scripts/identity/](scripts/identity/README.md) says where each is enforced and what a collision does.                    |
| **Principle**  | **Identity, ownership and provenance are three questions, and only the third answers "where did this page come from?"** A slug is not an identity, a route is not an identity, and an output path is not an identity — all three move when somebody renames something. A **source identity** does not: `wp:post/product#42` is WordPress's own primary key, and it is what a re-capture matches on. Every entity in `content/` states its origin, the manifest carries it to `dist/`, and `pnpm provenance` answers which WordPress entity produced a given URL or file. [scripts/provenance/](scripts/provenance/README.md) states the model and the twelve mutations that prove it. |
| **Principle**  | **WordPress's own taxonomies are first-class, and its media has a migration path.** Measured from core: `category` is hierarchical AND its rewrite is — `/category/parent/child/` — which the kit could not represent, because its category schema had no `parent`. Core `category` and `post_tag` are now derived profiles built from `permalinks`, so existing URLs are unchanged by construction and `content/categories.json` stays exactly where it is. Media went from an origin seam to a pipeline: `pnpm media discover                                                                                                                                                       | capture | verify`classifies every reference into six named outcomes, copies what it owns into`public/`, and rewrites `src`, `srcset`, lazy attributes, CSS `url()`and Markdown. A reference is migrated because of its HOST, never because it contains`/wp-content/uploads/`. [scripts/core-taxonomy/](scripts/core-taxonomy/README.md) and [scripts/media/](scripts/media/README.md). |
| **Limitation** | **Custom taxonomies are supported through a PROFILE, and their URL shape cannot be discovered.** Measured: `wp/v2/taxonomies` exposes no rewrite, and a taxonomy's name is not its URL base — `doc_category` serves terms at `/docs-category/`. So the prefix and whether it carries ancestors are configuration, checkable against a captured term's `link`. Supported: one `%term%` token, optional ancestor paths, several post-type collections per taxonomy, and stored-only taxonomies. **Locale-specific term URLs are not supported** — a term is one row named in each language. [scripts/taxonomies/](scripts/taxonomies/README.md) states the whole boundary.              |
| **Limitation** | **Custom post types are supported through a PROFILE, not automatically.** A type with a profile in `postTypes` gets a collection, an identity, a URL, a route and integrity coverage; a type without one is not routed and is reported by `content:census` as unconfigured. Supported patterns are `%postname%` with an optional fixed prefix and date parts. **Taxonomy-dependent URLs and taxonomy archives for a custom type are refused at build time by name** — the kit routes taxonomies for posts only. [scripts/custom-types/](scripts/custom-types/README.md) states the whole boundary.                                                                                    |
| **Limitation** | **The generic code assumes its stranger writes English in Latin script.** An audit before the black-box trial found nothing site-specific to any real domain in the engine — but slugs must be ASCII kebab-case (WordPress's `post_name` is not), and every string the kit itself writes — the 404 page, the skip link, breadcrumb "Home", and the `<meta name="description">` on every archive — is English with no seam. Word counting and search splitting assume whitespace-delimited words. [docs/04-implementation/stranger-site-boundaries.md](docs/04-implementation/stranger-site-boundaries.md) states each one with its evidence.                                          |
...
migration.config.ts     the source site: origin, permalinks, crawl pacing, route pairs,
                        which page builder built it, which custom post types it
                        publishes. Every field has a reader.
...
scripts/                the gate ladder — see scripts/README.md
  ...
  custom-types/         what the kit can do with a WordPress custom post type
  taxonomies/           what the kit can do with a WordPress taxonomy
  identity/             identity is not a slug — where collisions are caught
